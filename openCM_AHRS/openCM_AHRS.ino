//센서 1개로만 작동하는 방식 , 
//비례상수를 크게 하면 정확도는 증가하지만 매우 불안정해지며, 
//비례상수가 작아지면 안정적이지만 응답속도는 매우 느려지고 오차가 좀 커짐

/* MPU9250 
코드 원작자 :  Kris Winer
예제 코드를 바탕으로 opencm9.04에 맞게 수정함.
 
 /*
 편집자 : 박병관
for opencm9.04 C
  Hardware setup:
 MPU9250 Breakout --------- opencm 9.04 C
 VDD ----------------------- 3.3V
 SDA -----------------------0
 SCL ----------------------- 1
 GND ---------------------- GND
...
mpu-9250(9255)는 mpu-6050(6055)와 AK8963(지자기센서)을 하나로 묶은 형태이다.
mpu-6050(6055)는 자이로센서와 가속도센서를 융합시킨 센서이다.
FIFO(First In First Out) buffer  항상 최근값으로 덮어쓰는 버퍼
버퍼은 기본적으로는 가장 처음에 담긴 데이터(주소가 낮은 순)부터 선택하지만 FIFO버퍼는 항상 최근 담긴 데이터를 선택한다. 
 */
...
#define speed 512
#define Serialchart true
//#define processing
//#define mag_cailbration 
...
uint8 Gscale = GFS_250DPS; 
uint8 Ascale = AFS_2G;
uint8 Mscale = MFS_16BITS; // Choose either 14-bit or 16-bit magnetometer resolution
uint8 Mmode = 0x06;        // 2 for 8 Hz, 6 for 100 Hz continuous magnetometer data read
float aRes, gRes, mRes;      // scale resolutions per LSB for the sensors

// Pin definitions
int button1 = 16, button2 = 17;
int LED1 = 18, LED2 = 19, LED3 = 20;
...
int16 magCount[3];    // Stores the 16-bit signed magnetometer sensor output
 ...
 //mpu9250 magBias{170,40,220}
float magCalibration[3] = { 0, 0, 0}, magBias[3] = { 0, 0, 0}, magScale[3] = { 1, 1, 1 };  // Factory mag calibration and mag bias
float gyroBias[3] = { 0, 0, 0}, accelBias[3] = { 0, 0, 0};      // Bias corrections for gyro and accelerometer
int16 tempCount;      // temperature raw count output
...
uint32 lastUpdate= 0; // used to calculate integration interval
uint32 Now = 0;        // used to calculate integration interval

float ax, ay, az, gx, gy, gz, mx, my, mz; // variables to hold latest sensor data values 
float q[4] = { 1.0f, 0.0f, 0.0f, 0.0f };    // 사원수용 배열 선언
float eInt[3] = { 0.0f, 0.0f, 0.0f };       // vector to hold integral error for Mahony method
volatile float zeropoint[3]= {0.0f, 0.0f, 0.0f }; // 모터 영점각도
float motorangle[3]={0.0f, 0.0f, 0.0f };
...
void setup()
{
 ...
 pinMode(3, INPUT);  digitalWrite(3, LOW);  // Set up the interrupt pin, its set as active high, push-pull
  pinMode(button1, INPUT_PULLUP); // 485 button 1 
  pinMode(button2, INPUT_PULLUP); //485 button 2 
  
   pinMode(LED1,OUTPUT); pinMode(LED2,OUTPUT); pinMode(LED3,OUTPUT);  pinMode(BOARD_LED_PIN, OUTPUT);
  digitalWrite(LED1,HIGH); digitalWrite(LED2,HIGH); digitalWrite(LED3,HIGH);
  attachInterrupt(button1,motorpower,FALLING);
  attachInterrupt(button2,motorcalibration,FALLING);
  delay(500);
  digitalWrite(BOARD_LED_PIN, LOW);
  delay(500);
  // Display ID of sensor
  SerialUSB.println("-DOF 16-bit motion sensor ");
  delay(1000);

  // Read the WHO_AM_I register, this is a good test of communication
  byte c = readByte(MPU9250_ADDRESS, WHO_AM_I_MPU9250);  // Read WHO_AM_I register for MPU-9250
  SerialUSB.print("MPU9250 "); SerialUSB.print("I AM "); SerialUSB.print(c, HEX);
  SerialUSB.print(" I should be "); SerialUSB.print(0x71, HEX); SerialUSB.print("  or   ");  SerialUSB.println(0x73,HEX);
  
#ifndef mag_calibration
  magScale[0] = 0.99; magScale[1] = 1.05; magScale[2] = 0.97;
  magBias[0] = 119.62; magBias[1] = -439.07; magBias[2] =   409.90; 
#endif
       delay(1000); 
...
digitalWrite(BOARD_LED_PIN, HIGH);
    ...
    calibrateMPU9250(gyroBias, accelBias); // mpu-6050을 캘리브레이션하고 보정값을 센서에 입력한다.
    delay(1000); 
    initMPU9250(); 
...
{ 
 ...
 if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01) {  //인터럽 작동, 데이터 수집 확인용 인터럽
 ...
 getMres();  //지구자기장 단위 불러오기

    // Calculate the magnetometer values in milliGauss 
    // 공장초기값과 사용자 환경 초기값으로 보정
  ...
  sum += deltat; // sum for averaging filter update rate
    sumCount++;
    //mpu-6050의 x(y)-축은 지자기센서의 y(x)-축과 일치한다.
    //지자기센서의 +z축과 mpu-6050의 -z축은 지구 중심 방향이다.
  //사원수알고리즘에 입력하기 이전에 이러한 방향성 불일치를 해결해야한다.
  //따라서 mpu-9250에서 x축을 센서 전방으로 하기 위해서 지자기센서를 기준으로 잡았다. (LSM9DS0 도 동일)
  // in the LSM9DS0 sensor.
  ...
  //MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz,q);
            ...
            teapotPacket[3] = ((int)(q[0]*16384))<<8;
            teapotPacket[4] = ((int)(q[1]*16384))>>8;
            teapotPacket[5] = ((int)(q[1]*16384))<<8;
            teapotPacket[6] = ((int)(q[2]*16384))>>8;
            teapotPacket[7] = ((int)(q[2]*16384))<<8;
            teapotPacket[8] = ((int)(q[3]*16384))>>8;
            teapotPacket[9] = ((int)(q[3]*16384))<<8;
    ...
    pitch = (asin(2.0f * (q[0] * q[2]-q[1] * q[3])))*180.0f/PI; //raw pitch y
...
motorangle[1]=pitch-zeropoint[1];
    if(motorangle[1]>180){motorangle[1]-=360.0f;}
    if(motorangle[1]<-180){motorangle[1]+=360.0f;}
...
// 2Hz로 화면출력
 ...
 /*  if(state){
        SerialUSB.print("ax = "); 
        SerialUSB.print((float)1000*ax,2);  
        SerialUSB.print(" ay = "); 
        SerialUSB.print((float)1000*ay,2); 
        SerialUSB.print(" az = "); 
        SerialUSB.print((float)1000*az,2); 
        ...
        SerialUSB.print(" gz = "); 
        SerialUSB.print( gz, 2); 
        SerialUSB.println(" deg/s");
        SerialUSB.print("mx = "); 
        SerialUSB.print( (float)mx,2 ); 
        SerialUSB.print(" my = "); 
        SerialUSB.print( (float)my,2 ); 
        SerialUSB.print(" mz = "); 
        SerialUSB.print( (float)mz,2 ); 
        SerialUSB.println(" mG");
   }   */
#endif      
  /*
      // Define output variables from updated quaternion---these are Tait-Bryan angles (비행기 제어에 표준화된 모델)
      //  +z축은 지구 중심방향이다.
      // yaw는 (지자기센서)+x축과 자북극(보정이 정확하다면)사이의 각
      // 하이앵글에서는 CCW가 +값
      //  pitch는 (mpu6050) +x축과 지평선이 이루는 각. 축이 지구 중심이면 양수
      // roll은 (mpu6050) +y축과 지평선이 이루는 각이다. 축이 지구 중심방향이면 음수
      //mpu-9250(red)는 표시된 방향이 +방향 각을 가진다.
      //mpu-9255(blue)는 y축이 내장된 지자기센서의 x축과 일치한다.
      //이러한 방향성은 사원수에 의한 단위회전행렬에 의해 정의됨
      // 뿐만 아니라 오일러각 역시 비가환적이다. 따라서 반드시 정의된 순서대로 적용해야한다.
      //이 경우는 yaw(z)-pitch(y)-roll(x)순이다.
     ...
     */
      ...
      #if SerialDebug
         #ifndef processing
        SerialUSB.print("Yaw(Z), Pitch(Y), Roll(X): ");
        SerialUSB.print(motorangle[0], 2);
        SerialUSB.print(", ");
        SerialUSB.print(motorangle[1], 2);
        SerialUSB.print(", ");
        SerialUSB.println(motorangle[2], 2);
         #endif
        SerialUSB.println();
        SerialUSB.print("rate = "); 
        SerialUSB.print((float)sumCount/sum, 2); 
        SerialUSB.println(" Hz");
      #endif
      // 영일고등학교 좌표 (37°32'24"N 126°51'36"W) 는
  // 2017-07-14 기준  8° 22' W ± 0° 18' (또는 8.37°)  
...
void readAccelData(int16 * destination)
{
  ...
  readBytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]);  // Read the six raw data registers into data array // 일단 6바이트의 데이터를 요청한다. x(high) x(low) y(high) y(low) z(high) z(low)
  //데이터 수신의 단위는 죽었다 깨어나도 무조건 1바이트. 따라서 1바이트가 넘어가는 값은 반드시 데이터를 분할 시켜야 한다. 가장 쉬운 방법은 상위바이트와 하위바이트로 쪼개서 나누어서 전송하는 것이다.
  //그렇기 때문에 수신측에서는 2바이트만큼을 읽고 그 둘을 합쳐야 우리가 원하는 온전한 값을 받을 수 있음.
  destination[0] = ((int16)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value  첫번째로 읽은 1바이트는 상위 바이트 데이터 이므로 읽어서,
  //1바이트(8비트)를 비워서 하위바이트가 들어올 공간을 마련한 다음 OR(|)연산자로 하나의 데이터로 합친다.
  destination[1] = ((int16)rawData[2] << 8) | rawData[3] ;  //부호 있는 2바이트(16비트)짜리 정수를 읽어온다. 
  destination[2] = ((int16)rawData[4] << 8) | rawData[5] ; 
}
...
void readGyroData(int16 * destination)
{
  ...
  destination[0] = ((int16)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a signed 16-bit value
  destination[1] = ((int16)rawData[2] << 8) | rawData[3] ;  
  destination[2] = ((int16)rawData[4] << 8) | rawData[5] ; 
}

void readMagData(int16 * destination)
{
  uint8 rawData[7];  // x/y/z gyro register data, ST2 register stored here, must read ST2 at end of data acquisition
  if(readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
    readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);  // Read the six raw data and ST2 registers sequentially into data array
    uint8 c = rawData[6]; // End data read by reading ST2 register
    if(!(c & 0x08)) { // Check if magnetic sensor overflow set, if not then report data
      destination[0] = ((int16)rawData[1] << 8) | rawData[0] ;  // Turn the MSB and LSB into a signed 16-bit value
      destination[1] = ((int16)rawData[3] << 8) | rawData[2] ;  // Data stored as little Endian
      destination[2] = ((int16)rawData[5] << 8) | rawData[4] ; 
    }
  }
}
...
int16 readTempData()
{
  ...
  return ((int16)rawData[0] << 8) | rawData[1] ;  // Turn the MSB and LSB into a 16-bit value
}

void initAK8963(float * destination)
{
  // First extract the factory calibration for each magnetometer axis
  uint8 rawData[3];  // x/y/z gyro calibration data stored here
...
{  
  ...
  // DLPF_CFG = bits 2:0 = 011; this limits the sample rate to 1000 Hz for both (저주파통과필터 1KHz이하로 제한)
...
void MPU9250SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
  ...
  writeByte(MPU9250_ADDRESS, SMPLRT_DIV, 0x01);    // Set gyro sample rate to 1 kHz
...
void magCalMPU9250(float * bias_dest, float * scale_dest) //지자기 센서 사용자 환경 보정 함수
{
  ...
  SerialUSB.println("Mag Calibration: Wave device in a figure 8 until done!");
  SerialUSB.println("4 seconds to get ready followed by 15 seconds of sampling");
  delay(4000);
  ...
  SerialUSB.println("Mag Calibration done!");
       ...
       SerialUSB.println("mag biases x y z (mg)");
     SerialUSB.print(magBias[0]); SerialUSB.print("    "); 
     SerialUSB.print(magBias[1]); SerialUSB.print("    "); SerialUSB.println(magBias[2]);
     SerialUSB.println("mag Scale x y z (mg)");
     SerialUSB.print(magScale[0]); SerialUSB.print("    "); 
    SerialUSB.print(magScale[1]); SerialUSB.print("    "); SerialUSB.println(magScale[2]);
}

// Wire.h read and write protocols
void writeByte(uint8 address, uint8 subAddress, uint8 data)
{
  Wire.beginTransmission(address);  // 접속(송신버퍼 열기)
  Wire.write(subAddress);           // 서브주소 입력
  Wire.write(data);                 // 송신버퍼에 데이터 입력
  Wire.endTransmission();           // // 통신종료(주소값 전송 기능)
}

uint8 readByte(uint8 address, uint8 subAddress)
{
  uint8 data; // `data` will store the register data	 
  Wire.beginTransmission(address);         // 접속
  Wire.write(subAddress);	                 // 송신 버퍼에 주소기록
  Wire.endTransmission();      // 통신종료(주소값 전송 기능)
  Wire.beginTransmission(address);     // 데이터를 읽기 위해 재접속
  Wire.requestFrom(address, (uint8) 1);  // 서브 주소에서 데이터 요청
  data = Wire.read();                      // 수신데이터
  return data;                             // Return data read from slave register
}

void readBytes(uint8 address, uint8 subAddress, uint8 count, uint8 * dest)
{  
  Wire.beginTransmission(address);   // sensor 접속
  Wire.write(subAddress);            // 송신 버퍼에 서브주소기록
  Wire.endTransmission();   //통신종료(주소값 전송 기능)
  Wire.beginTransmission(address);     // 데이터를 읽기 위해 재접속
  uint8 i = 0;
  Wire.requestFrom(address, count);  // 서브 주소에서 데이터 요청
  while (Wire.available()) {
    dest[i++] = Wire.read(); 
  }         // 수신데이터->배열에 담기
}

void motorpower(void) //button1 interrupt 모터 제어기능 
 {  
   state=!state;
   digitalWrite(BOARD_LED_PIN,state);
  }
  
 void motorcalibration(void) // button2 interrupt 모터 캘리브레이션 
   {
  zeropoint[0] = yaw; 
  zeropoint[1] = pitch;
  zeropoint[2] = roll;
  ...
  int motormap(float angle) //각도를 모터 제어값으로 변환
  {
  int motor;  angle=constrain(angle,-150,150); return motor=map(angle,-150,150,0,1023);
  }
